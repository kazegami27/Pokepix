...
public class Purchaser : MonoBehaviour, IStoreListener
{
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        ...
        int index = Array.IndexOf(iapItems, item);
        if (index == 0)
        {
            CUtils.SetRemoveAds(true);
            Toast.instance.ShowMessage("Your purchase is successful", 2.5f);

            var removeAdSection = GameObject.FindWithTag("RemoveAdSection");
            if (removeAdSection != null) removeAdSection.SetActive(false);
        }
