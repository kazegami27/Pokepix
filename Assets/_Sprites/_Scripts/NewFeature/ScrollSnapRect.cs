...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    public GameObject contentacalXXX;
    public GameObject[] refrObj;
    public List<Text> Theme;

    private bool _horizontal;
    public List<Image> mark;
    ...
    private bool _lerp;
    private Vector2 _lerpTo = new Vector2(0, -97);
    ...
    private void SetPage(int aPageIndex) {
        ...
        foreach (Image image in mark)
        {
            image.color = Color.white;
        }
        if (mark.Count > 0 && Theme.Count > 0)
        {
            mark[aPageIndex].color = Color.red;
            Theme[aPageIndex].color = Color.red;
        }
        //StartCoroutine(DelayCall(aPageIndex));
    }

    IEnumerator DelayCall(int aPageIndex)
    {
        ...
        if (aPageIndex == 0)
        {
            ...
            _container.GetChild(aPageIndex).gameObject.SetActive(true);
            _container.GetChild(aPageIndex + 1).gameObject.SetActive(true);
        }
        else
        {
            foreach (Transform child in _container)
    ...
    private void LerpToPage(int aPageIndex)
    {
        ...
        if (mark.Count > 0 && Theme.Count > 0)
        {
            ...
            if (aPageIndex > 0)
                contentacalXXX.GetComponent<RectTransform>().LeanMoveLocalX(-refrObj[aPageIndex - 1].GetComponentInParent<RectTransform>().anchoredPosition.x, .5f);
            else
                contentacalXXX.GetComponent<RectTransform>().LeanMoveLocalX(0, 0.5f);
        }
        //if (aPageIndex == 0)
        //{
        //    foreach (Transform child in _container)
        ...
        //    }
        //    _container.GetChild(aPageIndex).gameObject.SetActive(true);
        //    StartCoroutine(LasyLoad(aPageIndex));
        //}
        //else if (aPageIndex > 0 && aPageIndex < _container.childCount)
        //{
        //    foreach (Transform child in _container)
        //    {
        //        child.gameObject.SetActive(false);
    ...
    IEnumerator LasyLoad(int aPageIndex)
    {
        ...
        else if (aPageIndex > 0 && aPageIndex < _container.childCount-1)
        {
            _container.GetChild(aPageIndex + 1).gameObject.SetActive(true);
            _container.GetChild(aPageIndex - 1).gameObject.SetActive(true);
        }
