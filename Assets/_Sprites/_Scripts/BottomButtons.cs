...
public class BottomButtons : MonoBehaviour {
    ...
    public void LoadButtons(List<Color> colors, BottomInfo info)
    {
        ...
        //info.width = info.cellSize ;
        scrollRect.GetComponent<RectTransform>().sizeDelta = new Vector2(info.width, info.cellSize * info.row + 30);
        ...
        for (int i = 0; i < info.numGrids; i++)
        {
            ...
            grid.transform.localPosition = new Vector2(info.width * i * 1.15f, 0);
            ...
            grid.GetComponent<RectTransform>().sizeDelta = new Vector2((info.cellSize + 30) * numButtons.Count, info.cellSize * info.row + 30);
        }
        scrollRect.content.GetComponent<RectTransform>().sizeDelta = new Vector2((info.cellSize+11) * numButtons.Count, info.cellSize * info.row + 30);
        if (selectedNumber != -1)
        {
