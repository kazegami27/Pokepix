...
public class Board : MonoBehaviour {
    ...
    public void OnEnable()
    {
        ...
        {
            LoadCompleteGame();
            GameController.instance.OnComplete(0, false);
        }
        else
        {
            LoadUncompleteGame();
    ...
    private BottomInfo GetBottomInfo()
    {
        ...
        info.width = mainCanvasTr.rect.width+info.cellSize;
        info.column = (int)Mathf.Ceil(info.width / 160f);
        info.column = Mathf.Max(100000, info.column);
    ...
    private void ZoomUpdate()
    {
        ...
        foreach (var tile in notPaintedTiles)
        {
            ...
            MinimMap.SetActive(enableText);
            button.SetActive(!enableText);
        }
    ...
    public void LogPoistion(string Name)
    {
        var subList = new List<Tile>();
        subList = notPaintedTiles.FindAll(x => x.number == refNum && !x.isPainted);
        if (Name == "Find")
        {
            //if (beingHighlights.Count >= 20)
            //{
            //var temp = UnityEngine.Random.Range(0, 20);
            boardCam.transform.position = new Vector3(beingHighlights[0].transform.position.x, beingHighlights[0].transform.position.y, -10);
            boardCam.orthographicSize = 1.2f;
            fixedCam.orthographicSize = boardCam.orthographicSize;
            ZoomUpdate();
            //}
            //else
            //{
            //    //var temp = UnityEngine.Random.Range(0, beingHighlights.Count);
            //    boardCam.transform.position = new Vector3(beingHighlights[0].transform.position.x, beingHighlights[0].transform.position.y, -10);
            //    boardCam.orthographicSize = .8f;
            //    fixedCam.orthographicSize = boardCam.orthographicSize;
            //    ZoomUpdate();
            //}
        }
        else if (Name == "PaintAll")
        {
            if (Test)
            {
            ...
            if (ModelControl.countPaint > 0)
            {
                ...
                else if (subList.Count > 0 && subList.Count <= 40)
                {
    ...
    public void UpdateUINumb()
    {
        if (ModelControl.countPaint > 0)
        {
            GameController.instance.PaintNumb.text = ModelControl.countPaint.ToString();
        }
        else
        {
            GameController.instance.PaintNumb.text = "AD";
        }
    }
    ...
    public void PaintTile(Tile tile)
    {
        ...
        if (GetSelectedNumber() == -1)
        {
            ...
            UpdateMywork();
            GameController.instance.OnComplete(0.9f, false);
